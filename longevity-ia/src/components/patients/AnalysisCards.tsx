...
import {
  ...
  Activity, TrendingUp, ArrowRight, FileText,
} from 'lucide-react'
  ...
  if (results.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center animate-fade-in">
        <div className="w-20 h-20 rounded-3xl bg-accent/10 border border-accent/20 flex items-center justify-center mb-6 animate-float">
          <FlaskConical size={32} className="text-accent" />
        </div>
        <h3 className="text-xl font-bold text-foreground mb-2">Sin estudios todavia</h3>
        <p className="text-muted-foreground max-w-sm mb-8">
          Sube tu primer estudio de laboratorio para que la IA genere tu analisis completo de longevidad.
        </p>
        <Link
          href={`/patients/${patient.id}/upload`}
          className="inline-flex items-center gap-2 bg-accent text-background font-semibold px-6 py-3 rounded-xl hover:bg-accent/90 transition-all shadow-accent/20 shadow-lg"
        >
          <Upload size={18} />
          Subir Estudio
        </Link>
      ...
      {results.map((r, i) => {
        ...
        return (
          <Link
            ...
            className="group card-medical p-0 overflow-hidden hover-lift transition-all duration-300 animate-slide-up"
