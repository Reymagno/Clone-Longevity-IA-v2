...
function NodeLabel({ name, cx, cy, r, dimmed }: {
  ...
  const fill = dimmed ? '#1A2E4C' : '#6B6660'
  ...
  return (
    <div className="card-medical overflow-hidden">
      ...
      <div className="p-5 space-y-4">
        ...
        {/* ── SVG Network ── */}
        <div className="relative rounded-xl border border-border bg-[#050E1B] overflow-hidden">
          ...
          >
            ...
            <rect x={-200} y={-200} width={W + 400} height={H + 400} fill="#050E1B" />
            {Array.from({ length: Math.ceil(W / 32) }).map((_, xi) =>
              Array.from({ length: Math.ceil(H / 32) }).map((_, yi) => (
                <circle key={`d-${xi}-${yi}`} cx={xi * 32 + 4} cy={yi * 32 + 4} r={0.6} fill="#1A2E4C" />
              ))
            )}
