...
const md = readFileSync(new URL('./LONGEVITY_IA_Manual_Ejecutivo.md', import.meta.url), 'utf-8')
...
for (let i = 0; i < lines.length; i++) {
  ...
  // Code blocks
  if (line.startsWith('```')) {
    // skip code fence lines, collect code content
    i++
    const codeLines = []
...
const outPath = new URL('./LONGEVITY_IA_Manual_Ejecutivo.docx', import.meta.url)
